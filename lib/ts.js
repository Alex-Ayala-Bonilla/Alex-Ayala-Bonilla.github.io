...
function parse(qx, qy, qz, N1, N2, N3) {
  ...
  if (qx_r.error ) {
    const m = `Invalid qx input: ${qx}`;
    error = m;    
    console.log(error);
  }

  if (qy_r.error ) {
    const m = `Invalid qy input: ${qy}`;
    error = m;    
    console.log(error);
  }

  if (qz_r.error ) {
    const m = `Invalid qz input: ${qz}`;
    error = m;    
    console.log(error);
  }
  ...
  return {T:Q, N1_unit_vector, N2_unit_vector,N3_unit_vector, N12_perp_proof,N13_perp_proof, N23_perp_proof,  rotValues, error , input: {qx, qy, qz, N1, N2, N3}}
}
